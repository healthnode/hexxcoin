...
//
// Bootup the masternode, look for a 10000 HXX input and register on the network
//
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    {
        if(out.tx->vout[out.i].nValue == Params().MasternodeValue()) { //exactly
...
vector<COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        if(out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == Params().MasternodeValue()) { //exactly
        	filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
