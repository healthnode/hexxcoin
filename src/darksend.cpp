...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeHXXAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fHXXSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f HXX CENT - sessionTotalValue %d\n", fHXXSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100 HXX+1 ( bit on if present )
    // bit 1 - 10 HXX+1
    // bit 2 - 1 HXX+1
    // bit 3 - .1 HXX+1
    // bit 3 - non-denom
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey){
    ...
    if(GetTransaction(vin.prevout.hash, txVin, hash)){
        BOOST_FOREACH(CTxOut out, txVin.vout){
            if(out.nValue == Params().MasternodeValue()) { // 500 * COIN){
                LogPrintf("vin correct: %d\n", out.nValue);
                if(out.scriptPubKey == payee2) {
                    LogPrintf("pubkey correct: %d\n", out.scriptPubKey.ToString().c_str());
                    return true;
                }
            }
        }
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeHXXAmount = (rand() % nLeftToAnon)+3;
                } else {
