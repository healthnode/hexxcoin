...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: hexx.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: hexx.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 29100 or testnet: 29300)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 29200 or testnet: 29400)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeHXXamount=<n> " + _("Keep N HXX anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Hexx is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Hexx is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Hexx version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Hexx server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Hexx") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Hexx to complete") << "\n";
    ...
    nAnonymizeHXXAmount = GetArg("-anonymizeHXXamount", 0);
    if(nAnonymizeHXXAmount > 999999) nAnonymizeHXXAmount = 999999;
    if(nAnonymizeHXXAmount < 2) nAnonymizeHXXAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeHXXAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize HXX Amount %d\n", nAnonymizeHXXAmount);
...
BOOST_FOREACH(PAIRTYPE(std::string, CMNodeConfig) node, pwalletMain->mapMyMNodes)
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CMNodeConfig) node, pwalletMain->mapMyMNodes)
	{
        uiInterface.NotifyMNodeChanged(node.second);
	}
