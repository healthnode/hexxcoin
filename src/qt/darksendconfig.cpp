...
void DarksendConfig::clickBasic()
{
    ...
    QMessageBox::information(this, tr("Darksend Configuration"),
        tr(
            "Darksend was successfully set to basic (%1 and 2 rounds). You can change this at any time by opening Hexx's configuration screen."
        ).arg(strAmount)
    );
...
void DarksendConfig::clickHigh()
{
    ...
    QMessageBox::information(this, tr("Darksend Configuration"),
        tr(
            "Darksend was successfully set to high (%1 and 8 rounds). You can change this at any time by opening Hexx's configuration screen."
        ).arg(strAmount)
    );
...
void DarksendConfig::configure(bool enabled, int coins, int rounds) {
    ...
    settings.setValue("nAnonymizeHXXAmount", coins);

    nDarksendRounds = rounds;
    nAnonymizeHXXAmount = coins;
}
