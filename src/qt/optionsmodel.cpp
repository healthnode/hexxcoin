...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeHXXAmount"))
        settings.setValue("nAnonymizeHXXAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeHXXAmount = settings.value("nAnonymizeHXXAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeHXXAmount"))
        SoftSetArg("-anonymizeHXXamount", settings.value("nAnonymizeHXXAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case anonymizeHXXAmount:
            nAnonymizeHXXAmount = value.toInt();
            settings.setValue("nAnonymizeHXXAmount", nAnonymizeHXXAmount);
            emit anonymizeHXXAmountChanged(nAnonymizeHXXAmount);
            break;
