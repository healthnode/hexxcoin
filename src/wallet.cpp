    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s HXX %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
                           ...
                           list<pair<CTxDestination, int64_t> >& listSent, int64_t& nFee, string& strSentAccount) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // CoinControl possible fix related... with HD wallet we need to report change?
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s HXX %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, AvailableCoinsType coin_type, bool useIX) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            int nDepth = pcoin->GetDepthInMainChain();
            if (nDepth <= 0) // coincontrol fix / ignore 0 confirm
                continue;
           ...
           /* for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && coin_type == ONLY_NONDENOMINATED_NOTMN) found = (pcoin->vout[i].nValue != Params().MasternodeValue()); // do not use MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
...
bool CWallet::SelectCoins(int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 HXX
    if(coin_type == ONLY_DENOMINATED){
        ...
        BOOST_FOREACH(int64_t v, darkSendDenominations)
        {
            ...
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN)+100 //round the amount up to .1 HXX over
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    uint64_t nCoinAge;
    {
        ...
        nReward = GetProofOfStakeReward(nCoinAge, nFees, false);
        if (nReward <= 0)
            return false;
    ...
    int64_t blockValue = nCredit + GetProofOfStakeReward(nCoinAge, nFees, hasPayment);
    int64_t masternodePayment = GetMasternodePayment(pindexPrev->nHeight+1, nReward);


    // Set output amount
    if (!hasPayment) // no masternode payment
    {
        // We want to maximize number of TxOuts with desirable sum.
        // So if blockValue greater then 1.5 AdvisablePoSTxOut we will
        // split it to equal parts AdvisablePoSTxOut size in such way
        // that last txout will be in range [0.5 : 1.0) * AdvisablePoSTxOut
        if( blockValue > Params().AdvisablePoSTxOut() * 3/2)
           {
               txNew.vout[1].nValue = Params().AdvisablePoSTxOut();
               int64_t residue = blockValue - Params().AdvisablePoSTxOut();
               int nTxOutForReward=1;
               while( residue > Params().AdvisablePoSTxOut() * 3/2)
                  {
                      residue -= Params().AdvisablePoSTxOut();
                      txNew.vout.push_back(CTxOut(0, scriptPubKeyOut));
                      nTxOutForReward +=1;
                      txNew.vout[nTxOutForReward].nValue = Params().AdvisablePoSTxOut();
                  }
               txNew.vout.push_back(CTxOut(0, scriptPubKeyOut));
               nTxOutForReward +=1;
               txNew.vout[nTxOutForReward].nValue = residue;
            }
        else
            txNew.vout[1].nValue = blockValue;
    }
    else // a masternode payment
    {
        //first txout is zero
        //second is PoS reward txout
        //third is for masternode
        //fourth and beyond again PoS reward
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s HXX %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s HXX %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
bool CWallet::AddMNodeConfig(CMNodeConfig nodeConfig)
{
    bool rv = CWalletDB(strWalletFile).WriteMNodeConfig(nodeConfig.sAlias, nodeConfig);
    if(rv)
    uiInterface.NotifyMNodeChanged(nodeConfig);
