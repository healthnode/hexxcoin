...
bool CWalletDB::WriteMNodeConfig(std::string sAlias, const CMNodeConfig& nodeConfig)
{
    nWalletDBUpdated++;
    return Write(std::make_pair(std::string("node"), sAlias), nodeConfig, true);
}

bool CWalletDB::ReadMNodeConfig(std::string sAlias, CMNodeConfig& nodeConfig)
{
    return Read(std::make_pair(std::string("node"), sAlias), nodeConfig);
}

bool CWalletDB::EraseMNodeConfig(std::string sAlias)
{
    nWalletDBUpdated++;
    return Erase(std::make_pair(std::string("node"), sAlias));
}
             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
        ...
        else if (strType == "node")
	{
	    std::string sAlias;
	    ssKey >> sAlias;
        CMNodeConfig MNodeConfig;
        ssValue >> MNodeConfig;
        pwallet->mapMyMNodes.insert(make_pair(sAlias, MNodeConfig));
	}
    } catch (...)
    {
        return false;
